            ...
            <div class="type2" v-if="menuType === 2">
                ...
                <el-card style="margin-bottom: 50px">
                    ...
                    <div style="display: inline-block">
                        이름 <i class="el-icon-check"></i>
                        전공학과 <i class="el-icon-check"></i>
                        자격점수 <i class="el-icon-check"></i>
                ...
                <div v-if="aiap === 1">
                    <el-card style="margin-bottom: 50px; margin-top: 30px; width: 250px; float: left">
                        <div slot="header">
                            <span>자기소개서 통일성</span>
                        </div>
                        <div style="display: inline-block">
                    ...
                    <el-card style="margin-bottom: 50px; margin-top: 30px; margin-left:30px; width: 250px; float: left">
                        <div slot="header">
                    ...
                    <el-card style="margin-bottom: 50px; margin-top: 30px; width: 800px; float: left">
                        <div slot="header">
                            <span>상세 내용</span>
                        </div>
                        <div style="display: inline-block">
            ...
            <div class="type5" v-if="menuType === 6">
                ...
                <div style="height: 200px; width: 730px; margin-top: 80px;">
                    <template v-if="varauth === true">
                        <el-card>
                            소속 : {{ans.slice(ans.indexOf(')')+1,ans.indexOf('소'))}}
                            군별 : {{ans.slice(ans.indexOf('별')+1,ans.indexOf('계'))}}
                            군번 : {{ans.slice(ans.indexOf('번')+1,ans.indexOf('성'))}}
                            이름 : {{ans.slice(ans.indexOf('번')+14,ans.indexOf('|'))}}
                        </el-card>
                    </template>
                </div>
            </div>
...
<script>
    ...
    function authForm(strs) {
        ...
        if (!(strs.includes("발행번호"))) return false;
        return true;
    }
    ...
    export default {
        ...
        methods: {
            ...
            aiapChanger() {
                ...
                if (key === '') open()
                const baseURI = 'https://language.googleapis.com/v1/documents:analyzeEntities?key=' + key;
                this.$http.post(`${baseURI}`, {
                    'document': {
                        'type': 'PLAIN_TEXT',
                        'content': '진행한 프로젝트로는 웹 프로그래밍이 있습니다. 데이터베이스와 프론트엔드, 백엔드 기능을 학습했습니다. 주요 프레임워크는 vue.js와 angular.js를 사용했습니다. 서버는 파이썬 기반 도커 서버를 사용했습니다. Java와 C언어, C++, Python 등의 언어를 사용할 수 있습니다.'
                    },
                })
                    .then((result) => {
            ...
            uploadImageSuccess(formData, index, fileList) {
                ...
                if (key === '') open()
                const ocrURI = 'https://vision.googleapis.com/v1/images:annotate?key=' + key;
                const req = {
                    "requests": [
                        {
                            "image": {
                ...
                this.$http.post(`${ocrURI}`, req)
                    .then((response) => {
                        this.ans = response.data.responses[0].textAnnotations[0].description.replace(/(\s*)/g, "");
